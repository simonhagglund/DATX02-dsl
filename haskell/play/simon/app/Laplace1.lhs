...
integ :: RealFloat a => a -> a -> (a -> a) -> a -- definite integral
\begin{code}
...
laplace :: (RealFloat a, RealFloat b) => (a -> a) -> (b -> b)
...
addition_invariant f g a b = laplace (\t -> a * f t + b * g t ) == \s -> a * laplace f s + b * laplace f g s
...
laplaceDerivative (D f) s n = s * ((laplace f) n ) - const (f 0)
...
--apply_both_sides :: RealFloat a => Double -> (a -> a) -> (a, a)
apply_both_sides s f' = \s -> (laplace (f' s), s * laplace (f' s) - f' 0)
...
\begin{code}
integ :: (RealFloat a, Enum a) => (a -> a) -> a -> a -> a
integ f start end = sum [ f n | n <- [start..end] ]
